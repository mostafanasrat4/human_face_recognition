  ...
  for i in range(n):
    ...
    for j in range(height):
      if len(fin) > 0:
        ...
        break
    if len(data) == 0 or len(data[0]) < DATUM_WIDTH-1:
...
""" def loadDigitTrainDataset(n):
  items = [ sum(datum.getPixels(), []) for datum in loadDataFile("data/digitdata/trainingimages", n,28,28) ]
  labels = loadLabelsFile("data/digitdata/traininglabels", n)
  return np.array(items), np.array(labels)

def loadDigitValidDataset(n):
  items = [ sum(datum.getPixels(), []) for datum in loadDataFile("data/digitdata/validationimages", n,28,28) ]
  labels = loadLabelsFile("data/digitdata/validationlabels", n)
  return np.array(items), np.array(labels)

def loadDigitTestDataset(n):
  items = [ sum(datum.getPixels(), []) for datum in loadDataFile("data/digitdata/testimages", n,28,28) ]
  labels = loadLabelsFile("data/digitdata/testlabels", n)
  return np.array(items), np.array(labels) """
